# -*- coding: utf-8 -*-
"""FeedbackPrize2-v12h-train-3.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/16XTWBqXvEe9Cnm6_6wxTBpEcRkXApIHA

# Version v12h
* Pseudo-label with version v1a
"""
...
class Config():
    ...
    debug = False
    model_name = 'v1b'
    run_pl = True    # True: use the pseudo-labels, False otherwise.
    if run_pl:
        base_model = 'v1a'
...
if cfg.processed_data:
    train_kfold = pd.read_csv(os.path.join(cfg.extra_data_dir, f'train_seed_{cfg.seed}.csv'), usecols = ['discourse_id', 'fold'])
    train = pd.read_csv(os.path.join(cfg.comp_data_dir, 'train.csv'))
    train = pd.merge(train, train_kfold, right_on = 'discourse_id', left_on = 'discourse_id')
    one_hot_label = np.zeros((train.shape[0], 3))
    one_hot_label[np.arange(train['discourse_effectiveness'].values.size), train['discourse_effectiveness'].map(cfg.label_map).values] = 1
    train[['Ineffective', 'Adequate', 'Effective']] = one_hot_label

    if cfg.run_pl:
        if cfg.env == 'colab':
            train_pl = pd.read_csv(os.path.join(cfg.model_dir, cfg.base_model.split('_')[0][:-1], cfg.base_model.split('_')[0][-1], f'train_pl_{cfg.training_folds[0]}_{cfg.base_model}.csv'))
        else:
            train_pl = pd.read_csv(os.path.join(cfg.pl_data , f'train_pl_{cfg.training_folds[0]}_{cfg.base_model}.csv'))
        train_pl['fold'] = -1
        train_pl['essay_id'] = train_pl['essay_id'].apply(lambda x: x.split('_')[0])
        train_pl = train_pl.set_index('essay_id')
        essay_ids = train_pl.index.unique()
        train_pl = train_pl.loc[essay_ids[~np.in1d(essay_ids, train['essay_id'].unique())]]
        train_pl = train_pl.reset_index()
        train_pl = train_pl[train.columns]
        train = pd.concat([train, train_pl]).reset_index(drop = True)
else:
    train = pd.read_csv(os.path.join(cfg.comp_data_dir, 'train.csv'))
...
def organize_df(df, id, cfg):
    ...
    label_list = df[['Ineffective', 'Adequate', 'Effective']].values.reshape(1,-1).tolist()
...
def prepare_dataset(example, cfg = cfg, mode = 'train'):
    ...
    label_list = [np.array(l).reshape(-1, 3) for l in example['label_list']]
...
class FeedbackPrize2_Model(nn.Module):
    def __init__(self, cfg):
        ...
        # GRU
        self.rnn = nn.GRU(input_size = cfg.config.hidden_size, hidden_size = cfg.config.hidden_size // 2,
                          bidirectional = True, batch_first = True, dropout = 0.1, num_layers = 1)
    ...
    def criterion(self, pred, true):
        if self.cfg.run_pl:
            loss = SoftCrossEntropyLoss(ignore_index = -100)(pred, true)
        else:
            mask = (true == -100).any(axis = -1)
            encoded_true = true.argmax(dim = -1)
            encoded_true = encoded_true.masked_fill_(mask, -100)
            loss = nn.CrossEntropyLoss(ignore_index = -100, label_smoothing = cfg.label_smoothing)(pred.permute(0, 2, 1), encoded_true)
        return loss
    ...
    for i, item in enumerate(tbar):
        ...
        if epoch >= cfg.start_val_epoch:
            if (i + 1) in val_schedule:
                ...
                true = np.array([item for sublist in [np.array(i).reshape(-1, 3).argmax(axis = 1) for i in val_df['label_list'].tolist()] for item in sublist])
                ...
                if score < best_score:
                    ...
                    ckp = os.path.join(cfg.model_dir, cfg.model_name.split('_')[0][:-1], cfg.model_name.split('_')[0][-1], f'_fold_{fold}.pt')
...
def training_loop(cfg, fold = 0):
    ...
    if cfg.mode == 'train':
        ...
        ckp = torch.load(os.path.join(cfg.model_dir, cfg.model_name.split('_')[0][:-1], cfg.model_name.split('_')[0][-1], f'_fold_{fold}.pt'), map_location = cfg.device)
        if cfg.use_swa:
            swa_model = AveragedModel(model)
        ...
        _, pred = valid_fn(cfg, model, valid_dataloader)
        true = np.array([item for sublist in [np.array(i).reshape(-1, 3).argmax(axis = 1) for i in val['label_list'].tolist()] for item in sublist])
        assert pred.shape[0] == len(true)
        valid[list(cfg.label_map.keys())[:3]] = pred
